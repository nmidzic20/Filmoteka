...
class TMDBklijent {
   ...
   async dohvatiPoster(id, poster_path)
   {
      let url = "https://image.tmdb.org/t/p/w600_and_h900_bestv2" + poster_path +"?api_key="+this.apiKljuc;
      console.log("URL DOHVATI POSTER " + url);
      try 
      {
         if(!ds.existsSync("../aplikacija/posteri"))
         {
            ds.mkdirSync("../aplikacija/posteri");
         } 
         let odgovor = await fetch(url);
         let spremnikPolje = await odgovor.arrayBuffer();
         let spremnik = Buffer.from(spremnikPolje);
         await ds.writeFile("../aplikacija/posteri/" + id + ".jpg", spremnik, () => console.log("Uspješno preuzimanje"));
      }
      catch (greska)
      {
